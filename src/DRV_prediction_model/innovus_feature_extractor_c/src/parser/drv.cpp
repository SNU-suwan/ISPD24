...
void Drv::parse(){
	...
	if(ifs.is_open()){
		...
		while(!ifs.eof()){
			...
			char *token=strtok(lineC,delimit);
			while(token != NULL){
				if(!is_starting_writing && linenum == 9){
					is_starting_writing = true;
					break;
				}
				else if(!is_starting_writing){
					...
					if (tmp_str == "Bounds"){
						...
						token=strtok(NULL,delimit);
						tmp_str.assign(token);
						double lx=stod(tmp_str.substr(0,tmp_str.size()-1));
						token=strtok(NULL,delimit);
						tmp_str.assign(token);
						double ly=stod(tmp_str);
						token=strtok(NULL,delimit);
						token=strtok(NULL,delimit);
						token=strtok(NULL,delimit);
						tmp_str.assign(token);
						double rx=stod(tmp_str.substr(0,tmp_str.size()-1));
						token=strtok(NULL,delimit);
						tmp_str.assign(token);
						double ry=stod(tmp_str);
						BBox<double> bbox(lx,ly,rx,ry);
						drv new_drv(idI,typeS,layerS,relatedNetsVS,bbox);
						drvV.push_back(new_drv);
						idI++;
