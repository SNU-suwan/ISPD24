...
public:
    ...
    FeatureExtractor(std::string name_, fs::path out_, fs::path cout_, Def& def_, Drv& drv_, double pat_, double gcell_, int Ysize_, int Xsize_, bool is_ugnet_) 
    : name(name_), outPath(out_), congPath(cout_), def(def_), drv(drv_), patSize(pat_), gcellSize(gcell_), patY(pat_ * Ysize_), patX(pat_ * Xsize_), gridY(gcell_ * Ysize_), gridX(gcell_ * Xsize_), is_ugnet(is_ugnet_)  {}
...
public:
	...
	Grid2D macroDensity;
    Grid2D localNet33Min, globalNet33Min, selfCrossingNet33Min, shortRudy33Min, longRudy33Min, pinDensity33MinM1, pinDensity33MinM2, minProximity33Min, pinAccessInfo33Min, unfriendly33Min;
    Grid2D localNet33Max, globalNet33Max, selfCrossingNet33Max, shortRudy33Max, longRudy33Max, pinDensity33MaxM1, pinDensity33MaxM2, minProximity33Max, pinAccessInfo33Max, unfriendly33Max;
    Grid2D macroDensity33Min, macroDensity33Max;
