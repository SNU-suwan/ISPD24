...
class BO_SW():
    def __init__(self,
                #func, 
                dimensions, base_estimator=None,
                n_calls=100, n_random_starts=None,
                n_initial_points=10,
                initial_point_generator="random",
                acq_func="gp_hedge", acq_optimizer="auto", x0=None, y0=None,
        ...
        self._noise=noise
        self._n_jobs=n_jobs
        self._model_queue_size=model_queue_size
        '''
        # Check params
        ...
        self.optimizer = Optimizer(dimensions, base_estimator,
                              n_initial_points=n_initial_points,
                              initial_point_generator=initial_point_generator,
                              n_jobs=n_jobs,
        ...
        if not all(len(p) == self.optimizer.space.n_dims for p in x0):
        ...
        if x0 and y0 is None:
            #print("[ERROR] You should import initial inputs.")
            y0 = list(map(func, x0))
            n_calls -= len(y0)
        ...
        if x0:
            ...
            self.result = self.optimizer.tell(x0, y0)
            self.result.specs = self.specs
            if eval_callbacks(callbacks, self.result):
                return self.result
            '''
            # Optimize
    ...
    def optimize(self,new_x,new_y):
        self.result = self.optimizer.tell(new_x,new_y)
        self.result.specs = self.specs
        '''
        if eval_callbacks(callbacks, result):
            return
